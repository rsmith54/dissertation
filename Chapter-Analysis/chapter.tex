...
\chapter[A search for squarks and gluinos in all hadronic final states with Recursive Jigsaw Reconstruction][Top of Page Title]{A search for squarks and gluinos in all hadronic final states with Recursive Jigsaw Reconstruction}
...
In ATLAS, this is done using \GEANTFour~\cite{Agostinelli:2002hh}.
...
This produces simulation datasets for the considered signal models and each background in the analysis.
...
The \textit{Good Runs List}~\cite{Golling:2011zy} is a centrally-maintained list of data runs which have been determined to be ``good for physics''.
...
We define a set of signal regions using the RJR variables of \Cref{sec:rjr_hadronic}.
...
Binning as finely as possible\footnotemark~ leads to overfitting to the fluctuations present in the signal and background events passing the signal region selections.
...
The signal region definitions are shown in \Cref{tab:squark_srs,tab:gluino_srs,tab:compressed_srs}.
...
The compressed regions target mass splittings with $m_{\text{sparticle}} - m_{\text{LSP}} \, \undertilde{<} \, 200 \GeV$.
...
This can be see as the ``crossover'' in the sparticle-LSP plane where the differences between squark and gluino production begin to manifest themselves.
...
The SRs follow the scaleless versus scaleful strategy, with SRG1 having the loosest scaleful cuts coupled with the strongest scaleless cuts, and the converse being true in SRG3.
...
\Crefname{chapter}{Appendix}{Appendix}%lazy hack
Control region distributions are shown for one squark, gluino, and compressed signal region, with the rest found in \Cref{app:cr_n-1_plots}.
...
N{\"a}ively, one might expect us to use \Zll as the control process, as \Zll events are well-measured.
Unfortunately, the \Zll branching ratio is about half of \Zvv, which necessitates loosening the control region selection significantly.
...
These features are shown in \Cref{fig:boson_pt_ratio} in simulated truth events.
...
Distributions of CR$\gamma$ in squark, gluino, and compressed regions are shown in \Cref{fig:CRY,fig:CRY_SRJigsawSRG1a_LastCut_CRY_minusone,fig:CRY_SRJigsawSRC1_LastCut_CRY_minusone}.
...
\end{align}
around the $W$ mass of $30 \GeV < m_T < 100 \GeV$.
...
Distributions of CRW in squark, gluino, and compressed regions are shown in \Cref{fig:CRW,fig:CRW_SRJigsawSRG1a_LastCut_CRW_minusone,fig:CRW_SRJigsawSRC1_LastCut_CRW_minusone}.
...
Hadronic or all dileptonic \ttbar events are less troublesome, as hadronic \ttbar events generally have low \met (and \HFnm{PP}{1}{1}) and will not pass the kinematic selections, while dileptonic \ttbar events have a lower cross-section and good reconstruction efficiency from the two leptons.
...
Distributions of CRT in squark, gluino, and compressed regions are shown in \Cref{fig:CRT,fig:CRT_SRJigsawSRG1a_LastCut_CRT_minusone,fig:CRT_SRJigsawSRC1_LastCut_CRT_minusone}.
...
Distributions of CRQ in squark, gluino, and compressed regions are shown in \Cref{fig:CRQ,fig:CRQ_SRJigsawSRG1a_LastCut_CRQ_minusone,fig:CRQ_SRJigsawSRC1_LastCut_CRQ_minusone}.
...
From simulation, this region at least 50\% pure in $Z$ events, which was considered enough to validate the $Z$ modeling considering the extreme portion of phase space.
...
In this case, we reimpose one of the two inverted cuts in VRQ with respect to the signal regions, to make each one closer to the SRs.
In VRQa (VRQb), we reimpose the \HFnm{PP}{1}{1} ($\Delta_{\mathrm{QCD}}$).
...
Each bin is the \textit{pull} of the validation region corresponding to a particular signal region.
This is defined
\begin{align}
...
The other is the uncertainty assigned to the jet smearing method, which is derived using the method in~\cite{SUSY-2011-20}.
...
The maximum likelihood fit described in \Cref{sec:maximum_likelihood} can be used with a variety of event count inputs.
...
We use the $CL_{\mathrm{S}}$ procedure~\cite{Feldman:1997qc}, to derive the observed and expected limits on the number of events from BSM phenomena in each signal region.
