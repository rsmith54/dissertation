...
Event cleaning vetos events which could be affected by noncollision background, noise bursts, or cosmic rays.

%The rest of the preselection is used for the signal region and control regions used for background estimation.
The rest of the preselection cuts selecte events using scale variables used by previous searches, which reduce the dataset to a manageable size.
...
Note \Cref{eq:bkg_est} can also be used to measure the contamination of a control region with another background, as measured by another control region.
...
The nuisance paramters are also determined by this procedure, but do not have a straightforward interpretation.
...
\end{tabular}
\end{table}
Comparing this set of selections with the signal regions \Cref{tab:squark_srs}, we see these are the loosest cuts among all squark signal regions.
...
Using the SRS1a example in \Cref{tab:crw_kinematic_selection}, we the same kinematic cuts applied as in CRW, but with the required $b$-jet instead of a $b$-jet veto.
