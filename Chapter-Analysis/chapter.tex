...
There are \order 100 simulated signal samples produced in the plane.}
...
\footnotetext{These SRs were defined before the entire collision dataset was produced, and thus needed to be robust to a range of LHC luminosity scenarios.}
...
We only note this here since we can also apply \Cref{eq:bkg_est} to measure the contamination of a control region with another background as well.
...
Distributions of CR$\gamma$ in squark, gluino, and compressed regions are shown in \Cref{fig:CRY_SRJigsawSRS1a_LastCut_CRY_minusone,fig:CRY_SRJigsawSRG1a_LastCut_CRY_minusone,fig:CRY_SRJigsawSRC1_LastCut_CRY_minusone}.
...
As with CRW, we need to loosen the cuts applied to CRT with respect to the signal region in order to gain sufficient expected data statistics \todo{this again}.
...
\todo{remove some of these plots???, but which ones?}
...
The other is the uncertainty is that assigned to the jet smearing method.\todo{JET SMEARING UNCERTAINTY}
