...
We now have well-motivated guesses for both boosts, which allow us write our original characteristic mass $M_R$ in terms of the lab frame variables, by application of these two Lorentz boosts to the energies of \Cref{eq:mr}:
\begin{align}
...
\subfloat[Squark pair decay tree]  {\includegraphics[width=.45\linewidth]{ATLAS-CONF-2016-078/fig_02a}}\Cref{subfig:disquark_decay_tree}
\subfloat[Gluino pair decay tree]  {\includegraphics[width=.45\linewidth]{ATLAS-CONF-2016-078/fig_02b}}\Cref{subfig:digluino_decay_tree} \\
\subfloat[Compressed decay tree]{\includegraphics[width=.45\linewidth]{ATLAS-CONF-2016-078/fig_02c}}\Cref{subfig:compressed_decay_tree}
\subfloat[Anti-QCD assembling decay tree]{\includegraphics[width=.45\linewidth]{ATLAS-CONF-2016-078_INT/QCD/RJRtree_SelfAssembling}}\Cref{subfig:self_assembling_decay_tree}
\end{figure}
...
The decay tree in \Cref{subfig:disquark_decay_tree} corresponds exactly to that expected from squark pair production, and matches very closely with the principles of the razor approach.
...
Instead, the strategy is generally to look for a large-scale initial state radiation (ISR) jet which is recoiling off the pair-produced sparticles' system.
...
We describe here the variables used in the search described in \Cref{ATLAS-CONF-2016-078}.
...
For the search for noncompressed squark pair production, we use will use the following set of RJR variables:
\begin{itemize}
...
\end{itemize}
First, we note that there is an implicit requirement that each hemisphere has at least one jet (to even reconstruct the \Pa and \Pb frames), these variables are implicitly using two or more jets, as we expect for squark pair production.
...
For the search for noncompressed gluino pair production, we use will use the following set of RJR variables.
