...
An illustration of a track with these parameters is shown in \Cref{fig:track_schematic}.
...
Due to the high quality of the silicon measurements in the pixel detector and SCT, these requirements give good track reconstruction efficiency, as seen in \Cref{fig:track_eff} for simulated events~\cite{Hamano:1489674}.
\begin{figure}
...
An example of this procedure for a simulation dijet event is shown in \Cref{fig:topocluster}.
\begin{figure}
...
A very convenient summary plot is shown in \Cref{fig:atlas_interactions}.
\begin{figure}
...
\footnotetext{
These are not all used for all objects, but it's conceptually useful to think of these different categories.
}
...
One begins the reconstruction of electromagnetic objects from the sliding-window clusters are reconstructed from the EM calorimeter.
...
At this scale, photons interact almost exclusively via pair-production in the presence of the detector material, as shown in \Cref{fig:photon_pair_production}~\cite{Agashe:2014kda}.
...
Electrons have a number of important backgrounds.
...
The efficiency for unconverted tight photons as a function of $\pt$ is shown in \Cref{fig:photon_eff}.
\begin{figure}
...
\end{equation}
The $\sigma_{\text{ID,MS}}$ in the denominator of \Cref{eq:muon_sig} are the uncertainties on the corresponding quantity from the numerator.
...
The reconstruction efficiency as a function of \pt can be seen for Medium muons in \Cref{fig:muon_eff}.
...
\end{aligned}
\end{equation}
In \Cref{eq:kt}, $k_{T,i}$ is the transverse momentum of $i$-th jet \textit{constituent} and $\Delta_{ij}$ is the angular distance \deltaR between the constituents.
...
The jet reconstruction algorithm is run on the collection of all topoclusters reconstructed as in \Cref{sec:topoclusters}.
...
The steps to derive the JES are shown in \Cref{fig:jes_correction_steps} and described here.
...
The combined JES uncertainty as a function of \pt is shown in \Cref{fig:jes_uncertainties}.
...
\footnotetext{Not exactly zero, since zero momentum tracks wouldn't have a well-defined $(\eta,\phi)$ coordinate, but set to a value obeying $p_{\text{T,track}}  << 400 \MeV = p_{\text{track,min}}$.
...
\footnotetext{N{\"a}ively, due to approximate isospin symmetry, about 2/3 of the hadrons will be charged and 1/3 will be neutral.}
...
Candidate \Zmm events are first required to pass a muon or electron trigger, as described in \Cref{tab:triggers}.
...
Distributions of TST \Emiss{x}, \Emiss{y}, and  \met from early 2015 data taking are shown in \Cref{fig:tst_met_zmumu}.

The \met resolution is an important variable due to the fact that the bulk of the distributions associated to \Emiss{x(y)} are Gaussian distributed~\cite{Aad:2016nrq}.
...
\Cref{fig:tst_met_resolution_zmumu} shows the TST \met resolution in the early 2015 data compared with simulation.
...
In \Cref{fig:tst_met_scale_zmumu}, the scale is shown for the early 2015 dataset.
...
\begin{figure}
\caption{TST \Emiss{x}, \Emiss{y}, and \met distributions of early $\sqrt{s} = 13 \TeV$ data compared with simulation after the \Zmm selection. The data sample consists of 6~\ipb.} \label{fig:tst_met_zmumu}
...
\Cref{fig:mc_algs_met_resolution_zmumu,fig:mc_algs_met_scale_zmumu} show the resolution and scale in simulated \Zmm events.
...
The charged PFOs (\order 2/3) are pileup suppressed, while the neutral PFOs (or topoclusters) are not.
...
The resolution and linearity are shown in simulated \Wen events in \Cref{fig:mc_algs_met_resolution_wenu}.
...
A comparison of the \met between real data and simulation for each algorithm is presented in \Cref{fig:data_algs_met_zmumu}.
The resolution as a function of \sumET and \npv~ is shown in \Cref{fig:data_algs_met_resolution_zmumu} for this dataset.
...
\subfloat[ ]{\includegraphics[width=.45\linewidth]{HCW_plots/resol_sumet}}
\subfloat[ ]{\includegraphics[width=.45\linewidth]{HCW_plots/resol_npv}}
\end{figure}
