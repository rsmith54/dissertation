...
For events with real \met, one can also look at the linearity in simulation
This is defined as
\begin{equation}
...
As stated earlier, the \met from the PFlow algorithm can be seen as a hybrid of TST \met and CST \met; the charged PFOs (\order 2/3) are pileup supressed, while the neutral PFOs (or topoclusters) are not.
...
For the nonsuppressed CST, the scale continues to worsen with increasing \ptZ; it is almost always the worst performing algorithm.
...
In this case, the pileup effects cancel, on average, due to the U$(1)_\phi$ symmetry of the ATLAS detector, and CST performs rather well compared to the more complicated track-based algorithms.\todo{check}
...
The scale in these events is best measured using a track-based soft term \todo{why???}.
...
This is likely due to simplifications made in the simulation; soft interactions that cannot be simulated can have a significant effect on an event level variable such as the \met resolution.\todo{check or think?}.
