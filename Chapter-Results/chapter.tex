...
This chapter presents the results of the search for squarks and gluinos in all hadronic final states.
...
\Cref{fig:srs_scale,fig:srg_scale,fig:src_scale} show the distributions of the last scale cut  (\ptisr, \HTFnm{PP}{4}{1}, or \HTFnm{PP}{2}{1}) used for each signal region.
...
As SRC5 is the gluino-like compressed signal region, its systematic uncertainties are similar to the gluino signal regions.
...
In \Cref{tab:p0_UL_RJR}, we show the one-sided $p$-value ($p_0$) and the equivalent statistical significance Z for each signal region:
\begin{align}
...
The model-independent limits are shown in \Cref{tab:p0_UL_RJR}.
...
We set limits in the plane of these free parameters.
...
The weaker observed limits are a result of this fluctuation.
